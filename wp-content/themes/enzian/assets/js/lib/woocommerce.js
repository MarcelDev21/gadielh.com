        ...
        $document
            ...
            .on('click', '.la_wishlist_table a.la_remove_from_wishlist', function(e){
                ...
                $table.load( e.target.href + ' #la_wishlist_table_wrapper2', function(){
                    ...
                    try {
                        if (Cookies.get('enzian_wishlist') !== 'undefined') {
                            const _cW = JSON.parse(Cookies.get('enzian_wishlist'));
                            $('.la-wishlist-count').html(_cW[0].posts.length)
                        }
                    } catch (ex) { }
                } );
            });
        ...
        $document
            ...
            .on('added_to_cart', function( e, fragments, cart_hash, $button ){
                if($button && $button.closest('.la_wishlist_table').length ) {
                    ...
                    $table.load( window.location.href + ' #la_wishlist_table_wrapper2', function (){
                        try {
                            if (Cookies.get('enzian_wishlist') !== 'undefined') {
                                const _cW = JSON.parse(Cookies.get('enzian_wishlist'));
                                $('.la-wishlist-count').html(_cW[0].posts.length)
        ...
        $document
            ...
            .on('click', '.la_remove_from_compare', function(e){
                ...
                $table.load( e.target.href + ' #la_compare_table_wrapper2', function(){
                    ...
                    try {
                        if (Cookies.get('enzian_compare') !== 'undefined') {
                            const _cW = JSON.parse(Cookies.get('enzian_compare'));
                            $('.la-compare-count').html(_cW[0].posts.length)
                        }
                    } catch (ex) { }
                } );
            });
    ...
    LaStudioWooCommerce.prototype.AjaxFilter = function (){
        ...
        function ajaxFilterSuccessCallback( response, url, element){
            ...
            $('.widget.lakit-wp--widget').each(function (){
                var _col_id = $(this).closest('.elementor-column').data('id');
            ...
            if(elementorSidebar.length > 0){
                elementorSidebar.forEach( function (_w){
                    if($(response).find('.elementor-column[data-id="'+_w+'"]').length){
                        $('.elementor-column[data-id="'+_w+'"]').replaceWith($(response).find('.elementor-column[data-id="'+_w+'"]'));
                        LaStudio.core.elementorFrontendInit( $('.elementor-column[data-id="'+_w+'"]') );
                        LaStudio.core.Blog( $('.elementor-column[data-id="'+_w+'"]') );
                    }
                } )
            }
