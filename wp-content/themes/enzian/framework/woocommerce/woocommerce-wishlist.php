...
if(!class_exists('Enzian_WooCommerce_Wishlist')){
    class Enzian_WooCommerce_Wishlist {
        ...
        public function ajax_process(){
            ...
            if(apply_filters('enzian/check_ajax_referer', false, 'wishlist_nonce', 'security')){
            ...
            if( $check_ajax_referer ) {
                if(!isset($_REQUEST['post_id'])){
                    wp_send_json_error(array(
                        'message' => esc_html__('Invalid Product ID', 'enzian')
                    ));
                }
                ...
                wp_send_json_error(array(
                    'message' => esc_html__('Invalid security key', 'enzian')
                ));
            }
        }
        ...
        private function add( $post_id = 0 ){
            $lists = $this->get_data();
            $response = array();
            $response['wishlist_url'] = enzian_get_wishlist_url();
            if(in_array($post_id, $lists)){
                $response['message'] = esc_html__('Product already in wishlist', 'enzian');
                $response['has_error'] = true;
            }
            else{
                $response['message'] = esc_html__('Product added', 'enzian');
